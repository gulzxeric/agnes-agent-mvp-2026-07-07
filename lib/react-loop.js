...
export async function runReActLoop(userInput, tools, maxIterations = 10) {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    let output;
    try {
      output = await callLLM(messages, { temperature: 0.3 });
    } catch (err) {
      console.error(`LLM call failed on iteration ${i + 1}:`, err.message);
      messages.push({
        role: 'user',
        content: `Observation: LLM 调用失败（${err.message}），请重试或换一种方式继续。`
      });
      continue;
    }
    ...
    try {
      ...
      const observation = `Observation: ${resultStr.length > 1000 ? resultStr.slice(0, 1000) + '...(truncated)' : resultStr}`;
